...
/*   Updated: 2024/10/24 15:24:11 by aklimchu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	eating(t_philo *philo, t_cur cur)
{
	if (lock_first_fork(philo, cur) == 1)
		return (1);	// unlocking mutexes before?

	if (lock_second_fork_and_eat(philo, cur) == 1)
		return (1); 	// unlocking mutexes before?

	pthread_mutex_lock(&philo->eaten_times_mutex[cur.philo_seat - 1]);
	philo->eaten_times[cur.philo_seat - 1]++; // or in the beginning of the meal?
	...
	/* pthread_mutex_unlock(&philo->eating_mutex[cur.philo_seat - 1]);
	pthread_mutex_unlock(&philo->eating_mutex[cur.philo_seat_previous - 1]); */
...
/* 	pthread_mutex_lock(&philo->eating_mutex[cur.philo_seat - 1]);
	pthread_mutex_lock(&philo->eating_mutex[cur.philo_seat_previous - 1]); */

	pthread_mutex_lock(&philo->fork_mutex[cur.fork_1 - 1]); // locking the first fork
	
	/* pthread_mutex_lock(&philo->state_mutex[cur.philo_seat - 1]);
	philo->state[cur.philo_seat - 1] = FIRST_FORK;
	pthread_mutex_unlock(&philo->state_mutex[cur.philo_seat - 1]); */
